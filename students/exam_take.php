<?php
// students/exam_take.php - Permanent exam UI (1 question per page, palette, calculator)
...
// exid is a STRING like "EX1001"
$exid = isset($_GET['exid']) ? trim($_GET['exid']) : '';
if ($exid === '') {
    die("Invalid EXID.");
}

// ---------------------------
// Fetch exam meta from exm_list
// Assumed columns: exid (PK, VARCHAR), exname, extime, subt, nq, desp, subject
// ---------------------------
$exam = null;
if ($stmt = $conn->prepare("SELECT exid, exname, extime, subt, nq FROM exm_list WHERE exid = ? LIMIT 1")) {
    $stmt->bind_param('s', $exid);  // <-- EXID IS STRING
    $stmt->execute();
    $res = $stmt->get_result();
...
if (!$exam) {
    die("Exam not found for EXID = " . htmlspecialchars($exid, ENT_QUOTES, 'UTF-8'));
}

// time left until submission deadline (subt)
$now     = new DateTime('now', new DateTimeZone('Asia/Kolkata'));
$subt    = new DateTime($exam['subt'], new DateTimeZone('Asia/Kolkata'));
$diffSec = $subt->getTimestamp() - $now->getTimestamp();

if ($diffSec <= 0) {
    die("This exam is already closed.");
}

// ---------------------------
// Fetch questions for this exam from qstn_list
// Assumed columns in qstn_list:
//   id (int PK)
//   exid (varchar FK to exm_list.exid)
//   qno (int, question number)
//   question (text)
//   opt1, opt2, opt3, opt4 (options)
//   ans (correct option index or text) - not needed here
// If your column names differ, change them ONLY in this query block.
// ---------------------------
$questions = [];

if ($stmt = $conn->prepare("
    SELECT id, qno, question, opt1, opt2, opt3, opt4
    FROM qstn_list
    WHERE exid = ?
    ORDER BY qno ASC
")) {
...
if (count($questions) === 0) {
    die("No questions found in qstn_list for EXID = " . htmlspecialchars($exid, ENT_QUOTES, 'UTF-8'));
}

// Pass questions to JS
$questionsJson = json_encode($questions, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);

// include your shared header (Bootstrap, container etc.)
    ...
    <div class="row g-3">
      ...
      <div class="col-lg-4">
        <div class="palette-card mb-3">
          ...
          <div id="questionPalette" class="mb-2">
            <!-- buttons added by JS -->
          </div>
...
// State per question
// answers[i] = option index (0..3) or null
// status[i]  = "not_visited" | "not_answered" | "answered" | "review"
...
function renderQuestion(idx) {
  ...
  questionNumberEl.textContent = `Question ${idx + 1} of ${totalQuestions}`;
...
function buildPalette() {
  ...
  for (let i = 0; i < totalQuestions; i++) {
    ...
    btn.textContent = questions[i].qno || (i + 1);
    btn.dataset.index = i;
...
function finalSubmit(force = false) {
  ...
  const payload = questions.map((q, i) => ({
    qid: q.id,
    qno: q.qno,
    selected: answers[i],   // option index or null
    status: status[i],
  }));
...
// -----------------------------
// Timer (based on subt from DB)
// -----------------------------
let secondsLeft = <?php echo max(1, $diffSec); ?>;
